...
This guide will walk you through installing and configuring the Degrando CSGO Egg for Pterodactyl.
...
- **Download Egg** from `pterodactyl/degrando-csgo-egg.json` (main branch)

### Step 2: Import the Egg

1. Log into your Pterodactyl Panel as an **administrator**
2. Navigate to **Admin** в†’ **Nests** (under Service Management)
...
If you have an existing CSGO server and want to switch to this egg:
...
1. Go to **Admin** в†’ **Servers**
2. Select your server
3. Navigate to the **Startup** tab
4. Change the **Nest** to the one containing the Degrando egg
5. Change the **Egg** to `Degrando CSGO Egg`
...
7. Select the **Docker Image** (see [Docker Image Registries](#docker-image-registries) below):
   - **Docker Hub** (Limited pulls):
     - `docker.io/degrando/csgo-egg:latest` (Stable)
     - `docker.io/degrando/csgo-egg:beta` (Beta)
   - **GitHub Container Registry** (Unlimited pulls):
     - `ghcr.io/degrando/csgo-egg:latest` (Stable)
     - `ghcr.io/degrando/csgo-egg:beta` (Beta)
8. Click **Save Modifications**
9. Restart your server

## Creating a New Server

1. Navigate to **Admin** в†’ **Servers**
2. Click **Create New**
3. Fill in the basic server details (name, owner, etc.)
4. Scroll down to **Nest Configuration**
5. Select the nest containing the Degrando egg
6. Select `Degrando CSGO Egg` as the egg
...
2. CSGO server files will be downloaded (~30GB)
...
The CSGO Egg is available from two container registries with different pull rate limits:
